...
'''
This code takes the data created from Make_IMDB_Data and assess the accuracy of classifiers used on
the data.  
'''
...
def ImportMeta(num=-1):
    ...
    '''
    match_file='IMDB/Results/imdbMeta.csv'
    metadata={}
...
def Analyze_Raw(metadata,cl,outfile,**CVargs):
    ...
    filename='IMDB/Data/IMDB_Raw_Scores.txt'
    ...
    for k,score in preds.iteritems():
        preds[k]=(score-m)/sd
    #fname='IMDB/Results/Raw_Preds.csv'
...
def Analyze_Individual(metadata,cl,outfile,**CVargs):
    ...
    #metadata=ImportMeta(-1)
    filename='IMDB/Data/IMDB_Individual_Scores.txt'
    data=ImportFeatureData(filename,-1)
    ...
    for k,score in preds.iteritems():
        preds[k]=(score-m)/sd
    #fname='IMDB/Results/Individual_Preds.csv'
...
def Analyze_LIWC(metadata,cl,outfile,**CVargs):
    filename='IMDB/Data/IMDB_LIWC_Scores.csv'
...
def Analyze_Raw_Topic_Scores(metadata,cl,outfile,**CVargs):
    ...
    #metadata=ImportMeta(-1)
    filename='IMDB/Data/Raw_Topic_Scores.csv'
    data=ImportCSVFeatureData(filename,-1)
    print 'drawing samples'
    vec=np.array([[float(l) for l in line[1:]] for line in data])   #exclude cases where sex is unknown
    labels=np.array([metadata[line[0]][0] for line in data])# if 'age' not in line])
    ...
    #print vec.shape
    #print labels
    #print np.mean([int(l) for l in labels])
    #print IDX
    Preds=Classifiers.CrossValidate(vec,labels,IDX,cl,**CVargs)
    print 'standardizing scores'
    preds={}
    ...
    for k,score in preds.iteritems():
        preds[k]=(score-m)/sd
    #fname='IMDB/Results/Raw_Topic_Preds.csv'
...
def Analyze_Nonword_Scores(metadata,cl,outfile,**CVargs):
    ...
    #metadata=ImportMeta(-1)
    filename='IMDB/Data/IMDB_Nonword_Scores.csv'
    data=ImportCSVFeatureData(filename,-1)
    print len(data), ' cases'
    print 'drawing samples'
    vec=np.array([[float(l) for l in line[1:]] for line in data])   #exclude cases where sex is unknown
    #vec[:,8]=vec[:,8]*-1    #herndanV is always neg (changed in Make_IMDB_Data now)
    #vec[:,12]=vec[:,12]*-1   #LnM is always neg (changed in Make_IMDB_Data now)
    labels=np.array([metadata[line[0]][0] for line in data])# if 'age' not in line])
    IDX=np.array([line[0] for line in data])
    ...
    for k,score in preds.iteritems():
        preds[k]=(score-m)/sd
    #fname='IMDB/Results/Nonwords_Preds.csv'
...
def Analyze_Structure_Scores(metadata,cl,outfile,**CVargs):
    ...
    #metadata=ImportMeta(-1)
    #Data/IMDB_Structure_'+cat+'_Scores.csv','wb')
    path='IMDB/Data/'
    PREDS=dict(zip(metadata.keys(),[[] for i in metadata.keys()]))
    for cat_id in xrange(len(metadata.values()[1][1:])):
        for cat in set([line[cat_id+1] for line in metadata.values()]):
            if cat=='category' or '_null' in cat:
                print cat
                continue
            args=CVargs.copy()
            print 'RUNNINING ', cat, ' STRUCTURE SCORES'
            f='IMDB_Structure_'+cat+'_Scores.csv'
    ...
    #fname='IMDB/Results/Structure_Preds.csv'
...
def Analyze_Behavior_Scores(metadata,cl,outfile,**CVargs):
    ...
    path='IMDB/Data/'
    PREDS=dict(zip(metadata.keys(),[[] for x in metadata]))
    for cat_id in xrange(len(metadata.values()[1][1:])):
        for cat in set([line[cat_id+1] for line in metadata.values()]):
            if cat=='category' or '_null' in cat:
                print cat
            ...
            f='IMDB_Behavior_'+cat+'_Scores.csv'
            data=ImportCSVFeatureData(path+f,-1)
            vec=np.array([[float(l) for l in line[1:]] for line in data])   #exclude cases where sex is unknown
            labels=np.array([metadata[line[0]][0] for line in data])# if 'age' not in line])
            IDX=np.array([line[0] for line in data])
    ...
    for k, scores in PREDS.iteritems():
        PREDS[k]=np.mean(scores)
    #fname='IMDB/Results/Behavior_Preds.csv'
...
def Analyze_SubTopic_Scores(metadata,cl,outfile,**CVargs):
    ...
    path='IMDB/Data/'
    PREDS=dict(zip(metadata.keys(),[[] for x in metadata]))
    print set([line[1] for line in metadata.values()])
    for cat_id in xrange(len(metadata.values()[1][1:])):
        for cat in set([line[cat_id+1] for line in metadata.values()]):
            if cat=='category' or '_null' in cat:
                print cat
    ...
    for k, scores in PREDS.iteritems():
        PREDS[k]=np.mean(scores)
    #fname='IMDB/Results/SubTopic_Preds.csv'
...
def Analyze_KBest_Scores(metadata,cl,outfile,**CVargs):
    filename='IMDB/Data/IMDB_KBest_Scores.csv'
    data=ImportCSVFeatureData(filename,-1)
    print 'drawing samples'
...
def Analyze_Word2Vec_Scores(metadata,cl,outfile,**CVargs):
    filename='IMDB/Data/IMDB_Word2Vec_Scores.csv'
    data=ImportCSVFeatureData(filename,-1)
    print 'drawing samples'
    vec=np.array([[float(l) for l in line[1:]] for line in data])   #exclude cases where sex is 
    labels=np.array([metadata[line[0]][0] for line in data])# if 'age' not in line])
...
#
#rtargs={'n_iter':20, 'test_size':.5,'random_state':0}
##cl=mnb()
##cl=svm.SVC(C=10,kernel='linear',probability=True)
#cl=ensemble.RandomForestClassifier(n_estimators=100)
#outfile='IMDB/rf/Raw_Topic_Preds.csv'
#Analyze_Raw_Topic_Scores(metadata,cl,outfile,**rtargs)
#
#print 'doing Nonwords'
#
#nwargs={'n_iter':20, 'test_size':.5,'random_state':0}
##cl=mnb()
##cl=svm.SVC(C=10,kernel='linear',probability=True)
#cl=ensemble.RandomForestClassifier(n_estimators=1000)
#outfile='IMDB/rf/Nonwords_Preds.csv'
#Analyze_Nonword_Scores(metadata,cl,outfile,**nwargs)

print 'doing Behavior'

bargs={'n_iter':10, 'test_size':.5,'random_state':0}
#cl=mnb()
#cl=svm.SVC(C=100,kernel='linear',probability=True)
cl=ensemble.RandomForestClassifier(n_estimators=10)
outfile='IMDB/rf/Behavior_Preds.csv'
Analyze_Behavior_Scores(metadata,cl,outfile,**bargs)

print 'doing Subtopics'

stargs={'n_iter':10, 'test_size':.5,'random_state':0}
#cl=mnb()
#cl=svm.SVC(C=10000,kernel='linear',probability=True)
cl=ensemble.RandomForestClassifier(n_estimators=10)
outfile='IMDB/rf/SubTopic_Preds.csv'
...
##cl=svm.SVC(C=10000,kernel='linear',probability=True)
#cl=ensemble.RandomForestClassifier(n_estimators=100)
#outfile='IMDB/rf/KBest_Preds.csv'
...
##cl=svm.SVC(C=10000,kernel='linear',probability=True)
#cl=ensemble.RandomForestClassifier(n_estimators=100)
#outfile='IMDB/rf/Word2Vec_Preds.csv'
...
##cl=svm.SVC(C=10,kernel='linear',probability=True)
##cl=ensemble.RandomForestClassifier(n_estimators=10)
#outfile='IMDB/nb/Individual_Preds.csv'
#Analyze_Individual(metadata,cl,outfile,**indargs)
#
#print 'doing LIWC'
#
...
##cl=svm.SVC(C=10,kernel='linear',probability=True)
##cl=ensemble.RandomForestClassifier(n_estimators=1000)
#outfile='IMDB/nb/LIWC_Preds.csv'
#Analyze_LIWC(metadata,cl,outfile,**liwcargs)
...
#cl=svm.SVC(C=10,kernel='linear',probability=True)
#cl=ensemble.RandomForestClassifier(n_estimators=10)
outfile='IMDB/nb/Structure_Preds.csv'
Analyze_Structure_Scores(metadata,cl,outfile,**strargs)     #Structure scores are accurate at around 60% using 50 words per cat.Will rerun to see if this imporves with more features.
#
#print 'doing Raw Topic'
#
...
##cl=svm.SVC(C=10,kernel='linear',probability=True)
##cl=ensemble.RandomForestClassifier(n_estimators=100)
#outfile='IMDB/nb/Raw_Topic_Preds.csv'
#Analyze_Raw_Topic_Scores(metadata,cl,outfile,**rtargs)
#
#print 'doing Nonwords'
#
...
##cl=svm.SVC(C=10,kernel='linear',probability=True)
##cl=ensemble.RandomForestClassifier(n_estimators=1000)
#outfile='IMDB/nb/Nonwords_Preds.csv'
#Analyze_Nonword_Scores(metadata,cl,outfile,**nwargs)
...
#cl=svm.SVC(C=100,kernel='linear',probability=True)
#cl=ensemble.RandomForestClassifier(n_estimators=10)
outfile='IMDB/nb/Behavior_Preds.csv'
Analyze_Behavior_Scores(metadata,cl,outfile,**bargs)
...
#cl=svm.SVC(C=10000,kernel='linear',probability=True)
#cl=ensemble.RandomForestClassifier(n_estimators=10)
outfile='IMDB/nb/SubTopic_Preds.csv'
Analyze_SubTopic_Scores(metadata,cl,outfile,**stargs)
#
#
#print 'doing KBest'
...
##cl=svm.SVC(C=10000,kernel='linear',probability=True)
##cl=ensemble.RandomForestClassifier(n_estimators=100)
#outfile='IMDB/nb/KBest_Preds.csv'
#Analyze_KBest_Scores(metadata,cl,outfile,**stargs)
#
#
#print 'doing Word2Vec'
...
##cl=svm.SVC(C=10000,kernel='linear',probability=True)
###cl=ensemble.RandomForestClassifier(n_estimators=100)
#outfile='IMDB/nb/Word2Vec_Preds.csv'
#Analyze_Word2Vec_Scores(metadata,cl,outfile,**stargs)
#
#
