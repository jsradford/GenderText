...
#fails:
    #subtopic
meta<-read.table('IMDB/Results/imdbMeta.csv',sep=',',header=T)
#GENERAL ANALYSIS FOR ALL DATA
beh<-read.table('IMDB/rf/Behavior_Preds.csv',sep=',' ,header=T)
raw<-read.table('IMDB/rf/Raw_Preds.csv',sep=',' ,header=T)
rt<-read.table('IMDB/rf/Raw_Topic_Preds.csv',sep=',' ,header=T)
struct<-read.table('IMDB/rf/Structure_Preds.csv',sep=',' ,header=T)
st<-read.table('IMDB/rf/SubTopic_Preds.csv',sep=',' ,header=T)
liwc<-read.table('IMDB/rf/LIWC_Preds.csv',sep=',' ,header=T)
w2v<-read.table('IMDB/rf/Word2Vec_Preds.csv',sep=',' ,header=T)
kb<-read.table('IMDB/rf/KBest_Preds.csv',sep=',' ,header=T)
nw<-read.table('IMDB/rf/Nonwords_Preds.csv',sep=',' ,header=T)
ind<-read.table('IMDB/rf/Individual_Preds.csv',sep=',' ,header=T)


library(plyr)
dta<-join_all(list(meta,beh,raw,rt,struct,st,liwc,w2v,kb,nw,ind), by = 'id', type = 'full')
colnames(dta)<-c(names(meta),'beh','raw','rawtopic','struct','subtopic','liwc','w2v','kbest','nonwords','individual')
...
saveImportanceChart<-function(fit,file){
    ...
    vars<-rownames(mat)
    jpeg(file)
    barplot(vals, main="IMDB Variable Importance",xlab='Model',ylab='Change in Accuracy',names.arg=vars)
    dev.off()
}


####UNBALANCED DATA with fixed categories (no "_null") and an "Other" category
names<-c('raw','rawtopic','w2v','kbest','liwc','nonword','behav.','struct.','subtop.','indiv')
counts<-c(.73,.70,.71,.77,.67,.68,.74,.67,.68,.92)
jpeg('IMDB/Results/IMDB_Accuracy_Bar.jpg')
barplot(counts, main="IMDB Classifier Accuracy",ylim=c(0,1),xlab='Model',ylab='Accuracy', cex.names=.6,
  names.arg=names)
dev.off()
...
####UNBALANCED DATA:
names<-c('raw','rawtopic','w2v','kbest','liwc','nonword','behav.','struct.','subtop.','indiv')
counts<-c(.73,.70,.71,.77,.67,.68,.70,.67,.68,.93)
jpeg('IMDB/Results/IMDB_Accuracy_Bar.jpg')
barplot(counts, main="IMDB Classifier Accuracy",ylim=c(0,1),xlab='Model',ylab='Accuracy', cex.names=.6,
  names.arg=names)
dev.off()
...
####BALANCED DATA:
names<-c('raw','rawtopic','w2v','kbest','liwc','nonword','behav.','struct.','subtop.','indiv')
counts<-c(.68,.62,.65,.71,.52,.58,.62,.48,.59,.91)
jpeg('IMDB/Results/IMDB_Accuracy_Bar.jpg')
barplot(counts, main="IMDB Classifier Accuracy",ylim=c(0,1),xlab='Model',ylab='Accuracy', cex.names=.6,
  names.arg=names)
dev.off()
