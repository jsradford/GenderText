...
NOTE: To make this code readable, it presumes the values for each class variable are informative: I.e. sex is "male/female" not 1/0
and poverty is "high/med/low" not 3,2,1.
...
def unzip(num=-1):
    ...
    '''
    #essayDir='C:\DonorsChoose\Data\MatchedEssays2'
    #source_filename='C:\Data\GenderText\Datasets\DonorsChooseData\\blogs.zip'
    #exact_file='DonorsChoose/ExactTeachers.csv'
    essay_file='Datasets/DonorsChoose/Essays.csv'
    #match_file='DonorsChoose/ExactTeacherStrata.csv'
    class_file='DonorsChoose/DonorsChooseClassData.csv'
    ...
    with open(class_file,'rb') as f:
        ...
        for i,row in enumerate(matches):
            if i==0:
                header=['id','sex']+row[2:]
            #if row[0][0:4] not in keys.keys():
            #    continue
            #if row[0] not in keys[row[0][0:4]]:
            #    continue
            metadata[row[0]]=[genDict[row[1]]]+row[2:]
            if row[0][0:4] in keys.keys():
                keys[row[0][0:4]].append(row[0])
            else:
                keys[row[0][0:4]]=[row[0]]
    print 'pulled out ', len(metadata), ' essays for comparison'
    texts={}
    htexts={}
    print 'getting essays'
    with open(essay_file,'rb') as f:
        essays=csv.reader(f,delimiter=',')
        for i,row in enumerate(essays):
            ...
            if row[0] not in keys[row[0][0:4]]:
                continue
            text='\n'.join(unicode(row[2:6]))
            #if len(text)<200:
            #    continue
            
            t = text.lower()
            t = re.findall(r"[\d\w']+",t)
            #set a minumum of 10 words
            if len(t)<10:
                metadata.pop(row[0])
                continue
            texts[row[0]]=text
            if len(texts)==num:
                break
    print 'pulled out ', len(texts), ' essays '
    
    print 'saving meta-data file'
    with open('DonorsChoose/Results/DonorsChooseMeta.csv','wb') as f:
...
def MakeLIWCData(texts):
    ...
    with open('DonorsChoose/Data/DonorsChoose_LIWC_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
        writer.writerow(header)
...
def MakeRawScore(texts,num_features=5000):
    ...
    outfile=open('DonorsChoose/Data/DonorsChoose_Raw_Scores.txt','w')
...
def MakeKBestScore(texts,metadata,num_features=5000,**tfkwargs):
    ...
    #    try:
    vec,labels,vectorizer=Vectorize.tfidfVectorize(texts,labels)#{k,re.findall(r"[\d\w']+",v)},labels[k],**tfkwargs)
        #except:
        #    print k,v
    ...
    sys.stdout.flush()
    print "writing KBest scores for DonorsChoose"
    with open('DonorsChoose/Data/DonorsChoose_KBest_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
def RawTopicScore(texts,numtopics=200,iterations=500,passes=10,name='DonorsChoose_Raw',**exargs):
    '''
    ...
    corpora.MmCorpus.serialize('DonorsChoose/Data/'+name+'_Corpus.mm', corpusgenerator) 
    print 'vectorizing ', name
    tfidf_corpus,tfidf,corpus=TopicModels.vectorize(toktexts,dictionary)
    print 'Doing lda ', name
    mm = corpora.MmCorpus('DonorsChoose/Data/'+name+'_Corpus.mm',)
    ...
    with open('DonorsChoose/Data/'+name+'_Topic_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
    ...
    with open('DonorsChoose/Results/'+name+'_TopicsByWords_'+str(numtopics)+'.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
def subTopicsScore(texts,metadata,numtopics=10,name='DonorsChoose'):
...
#d=subTopicsScore(texts,metadata,numtopics=10,name='DonorsChoose')

def MakeStructureScore(texts, metadata,num=10):
    '''
    Structure score is the model that best fits predictions of secondary infromation (in DonorsChoose Data, this is age and industry)
    ...
    for cats in xrange(len(matchData[0][2:])):
        for cat in set([line[cats+2] for line in matchData]):
            ...
            with open('DonorsChoose/Data/DonorsChoose_Structure_'+cat+'_Scores.csv','wb') as f:
                writer=csv.writer(f,delimiter=',')
...
def makeBehaviorScore(texts, metadata,num=10):
    ...
    #for i,cat in enumerate(categories):
            ...
            with open('DonorsChoose/Data/DonorsChoose_Behavior_'+cat+'_Scores.csv','wb') as f:
                writer=csv.writer(f,delimiter=',')
...
def makeIndivScore(texts,metadata,window=30):
    ...
    outfile=open('DonorsChoose/Data/DonorsChoose_Individual_Scores.txt','w')
...
def makeWord2Vec(texts,num_features,**kwargs):
    ...
    model_name = "donorschoose_300features_40minwords_10context"
    ...
    with open('DonorsChoose/Data/DonorsChoose_Word2Vec_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
def makeNonWordScore(texts,metadata):
    ...
    for key,text in texts.iteritems():
        ...
        #
    with open('DonorsChoose/Data/DonorsChoose_Nonword_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
#subTopicsScore(texts,metadata,numtopics=20,name='DonorsChoose')
#num_features=300
#kwargs={'size':num_features,'min_count':2,'workers':4,'window':10,'sample':1e-3}
#res=makeWord2Vec(texts,num_features,**kwargs)
#tfkwargs={'ngram_range':(1,4),'max_df':.9,'min_df':1,'stop_words':None}
MakeKBestScore(texts,metadata,num_features=10000)#,**tfkwargs)
