...
This was copied from an outdated Make_Abstracts_Data.py.  Update all functions to fit updated code from GenderText/Make_Abstracts_Data.py 
...
def unzip(num=-1):
    '''
    We're just testing this one now
    ...
    character_file='C:\Data\GenderText\Datasets\Abstracts\cornell movie-dialogs corpus\movie_characters_metadata.txt'
    character_file='/home/jsradford/GenderText/Datasets/Abstracts/cornell movie-dialogs corpus/movie_characters_metadata.txt'
    movie_meta_file='C:\Data\GenderText\Datasets\Abstracts\cornell movie-dialogs corpus\movie_titles_metadata.txt'
    movie_meta_file='/home/jsradford/GenderText/Datasets/Abstracts/cornell movie-dialogs corpus/movie_titles_metadata.txt'
    lines_file='C:\Data\GenderText\Datasets\Abstracts\cornell movie-dialogs corpus\movie_lines.txt'
    lines_file='/home/jsradford/GenderText/Datasets/Abstracts/cornell movie-dialogs corpus/movie_lines.txt'
...
tar='C:/Data/GenderText/Datasets/Abstracts/hep-th-abs.tar.gz'
noauthors=[]
raw=[]
metadata={}
texts={}
...
with tarfile.open(tar, mode="r") as archive:
    for f in archive:
        if f.isreg() and f.name.endswith(".abs"):
            #print 'doing ', f
            with closing(archive.extractfile(f)) as data:
                abstract=data.read()
                meta=abstract.split('\\\\')[1].split('\n')
                metas.append(meta)
                abstracts.append(abstract.split('\\\\')[2])
                gender='?'
                for line in meta:
                    if line.startswith('From:'):
                        ...
                        for item in fr:
                            if '@' in item: continue
                            names=re.findall(r'[a-zA-Z]+',item)
                            for name in names:
                                if gender!='?':
                                    continue
                                if len(name)==1:
...
Counter(genders)        
    ...
    with open(movie_meta_file,'rb') as f:
        ...
        for line in raw:
            ...
            moviedata[line[0]]=line[1:5]+[genre]
            genres.update(genre)
...
def MakeLIWCData(texts):
    ...
    #import /home/jsradford/GenderText/Scripts/Generic/Vectorize.py
    #import '/home/jsradford/GenderText/Scripts/Generic/FeatureSelection.py'
    #reload(FeatureSelection)
    #reload(Vectorize)
    #runfile('/home/jsradford/GenderText/Scripts/Generic/FeatureSelection.py')
    ...
    filename='C:\Abstracts\Data\NoGenders.csv'
    liwcTexts=LIWCize(texts)
    print 'vectorizing'
    vec,labels,vectorizer=Vectorize.tfidfVectorize(liwcTexts,dict([(k,'') for k in texts.keys()]))
    outfile=open('/home/jsradford/GenderText/Abstracts/Data/Abstracts_LIWC_Scores.txt','w')
...
def MakeRawScore(texts,num_features=5000):
    ...
    outfile=open('/home/jsradford/GenderText/Abstracts/Data/Abstracts_Raw_Scores.txt','w')
...
def RawTopicScore(texts,numtopics=200,iterations=500,passes=10,name='Abstracts_Raw',**exargs):
    '''
    ...
    #runfile('/home/jsradford/GenderText/Scripts/Generic/TopicModels.py')
    import TopicModels
    reload(TopicModels)
    ...
    corpora.MmCorpus.serialize('/home/jsradford/GenderText/Abstracts/Data/'+name+'_Corpus.mm', corpusgenerator) 
    print 'vectorizing ', name
    tfidf_corpus,tfidf,corpus=TopicModels.vectorize(toktexts,dictionary)
    print 'Doing lda ', name
    mm = corpora.MmCorpus('/home/jsradford/GenderText/Abstracts/Data/'+name+'_Corpus.mm',)
    ...
    with open('/home/jsradford/GenderText/Abstracts/Data/'+name+'_Topic_Scores.csv','wb') as f:
    ...
    with open('/home/jsradford/GenderText/Abstracts/Results/'+name+'_TopicsByWords_'+str(numtopics)+'.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
def subTopicsScore(texts,metadata,numtopics=10,name='Abstracts'):
...
#d=subTopicsScore(texts,metadata,numtopics=10,name='Abstracts')
...
def MakeStructureScore(texts, metadata,num=10):
    '''
    Structure score is the model that best fits predictions of secondary infromation (in Abstracts Data, this is age and industry)
    ...
    for cats in xrange(len(matchData[0][2:])):
        for cat in set([line[cats+2] for line in matchData]):
            ...
            with open('/home/jsradford/GenderText/Abstracts/Data/Abstracts_Structure_'+cat+'_Scores.csv','wb') as f:
                writer=csv.writer(f,delimiter=',')
...
def makeBehaviorScore(texts, metadata,num=10):
    ...
    #for i,cat in enumerate(categories):
            ...
            with open('/home/jsradford/GenderText/Abstracts/Data/Abstracts_Behavior_'+cat+'_Scores.csv','wb') as f:
                writer=csv.writer(f,delimiter=',')
...
def makeIndivScore(texts,metadata,window=30):
    ...
    outfile=open('/home/jsradford/GenderText/Abstracts/Data/Abstracts_Individual_Scores.txt','w')
...
def makeNonWordScore(texts,metadata):
    ...
    at D:\Users\Administrator\Documents\Chicago\Research\Gender on Abstracts\Readings\Computational Gender
    ...
    for key,text in texts.iteritems():
        ...
        #
    with open('/home/jsradford/GenderText/Abstracts/Data/Abstracts_Nonword_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
subTopicsScore(texts,metadata,numtopics=20,name='Abstracts')
