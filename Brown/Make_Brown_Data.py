...
This was copied from an outdated Make_Brown_Data.py.  Update all functions to fit updated code from GenderText/Make_Brown_Data.py 
...
def unzip(num=-1):
    ...
    categories={'a': ' press: reportage',
        ...
        'r': ' humor'}
    import csv
    catTrans={True:'nonfiction',False:'fiction'}
    index={}
    namedict=importNameDict()
    meta='Datasets/Brown/BrownMeta.csv'
    with open(meta,'rb') as f:
        data=csv.reader(f)
        for i,line in enumerate(data):
            if i>0:
                cat=catTrans['k'>=line[2].lower()[1]]
                code=line[2].lower()
                author=line[0]
                index[code]=[author,cat]
                #categories[code]=categories[cat]
    text={}
    metadata={}
    from nltk.corpus import brown
    for idx in brown.fileids():
        if idx not in index.keys():
            continue
        gender='?'
        names=re.findall(r'[a-zA-Z]+',index[idx][0].lower())
        for name in names:
            if gender!='?':
                continue
        ...
        if gender=='?':
            ...
            continue
        wordlist=brown.words(idx)
        text[idx]=' '.join(wordlist)
        metadata[idx]=[gender, index[idx][1]]
    
    print 'saving meta-data file'
    with open('Brown/Results/brownMeta.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
        writer.writerow(['id','sex','genre'])
...
def extractKBest(vec,labels,vectorizer,num):
...
def MakeLIWCData(texts):
    ...
    #outfile=open('Brown/Data/Brown_LIWC_Scores.csv','w')
    ...
    with open('Brown/Data/Brown_LIWC_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
def MakeRawScore(texts,num_features=5000):
    ...
    outfile=open('Brown/Data/Brown_Raw_Scores.txt','w')
...
def MakeKBestScore(texts,metadata,num_features=5000,**tfkwargs):
    ...
    sys.stdout.flush()
    print "writing KBest scores for Brown"
    with open('Brown/Data/Brown_KBest_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
def RawTopicScore(texts,numtopics=200,iterations=500,passes=10,name='Brown_Raw',**exargs):
    '''
    ...
    corpora.MmCorpus.serialize('Brown/Data/'+name+'_Corpus.mm', corpusgenerator) 
    print 'vectorizing ', name
    tfidf_corpus,tfidf,corpus=TopicModels.vectorize(toktexts,dictionary)
    print 'Doing lda ', name
    mm = corpora.MmCorpus('Brown/Data/'+name+'_Corpus.mm',)
    ...
    with open('Brown/Data/'+name+'_Topic_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
    ...
    with open('Brown/Results/'+name+'_TopicsByWords_'+str(numtopics)+'.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
def subTopicsScore(texts,metadata,numtopics=10,name='Brown'):
...
#d=subTopicsScore(texts,metadata,numtopics=10,name='Brown')
...
def MakeStructureScore(texts, metadata,num=10):
    ...
    What words most distinguish the categories and do men and women use these words at different rates.
    '''
    ...
    for cats in xrange(len(matchData[0][2:])):
        for cat in set([line[cats+2] for line in matchData]):
            ...
            with open('Brown/Data/Brown_Structure_'+cat+'_Scores.csv','wb') as f:
                writer=csv.writer(f,delimiter=',')
...
def makeBehaviorScore(texts, metadata,num=10):
    '''
    Behavior score is the model that best fits predictions of secondary infromation (in Blogger Data, this is age and industry)
    Do men and women within an industry talk about different things?
    '''
    from gensim import corpora, models, similarities
    import sklearn
    ...
    #for i,cat in enumerate(categories):
            ...
            with open('Brown/Data/Brown_Behavior_'+cat+'_Scores.csv','wb') as f:
                writer=csv.writer(f,delimiter=',')
...
def makeIndivScore(texts,metadata,window=30):
    ...
    outfile=open('Brown/Data/Brown_Individual_Scores.txt','w')
...
def makeWord2Vec(texts,num_features,**kwargs):
    ...
    model_name = "brown_300features_40minwords_10context"
    ...
    with open('Brown/Data/Brown_Word2Vec_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
def makeNonWordScore(texts,metadata):
    ...
    at D:\Users\Administrator\Documents\Chicago\Research\Gender on Brown\Readings\Computational Gender
    ...
    for key,text in texts.iteritems():
        ...
        #
    with open('Brown/Data/Brown_Nonword_Scores.csv','wb') as f:
        writer=csv.writer(f,delimiter=',')
...
def QuickFit(res,metadata,texts):
    runfile('Scripts/Generic/Classifiers.py')
    from sklearn import ensemble
    IDX=np.array([i for i,line in enumerate(res)])
    vec=np.array([[float(l) for l in line[1:]] for line in res])
    labels=np.array([metadata[texts.keys()[i]][0] for i,line in enumerate(res)])
    cl=ensemble.RandomForestClassifier(n_estimators=10)
    
    Preds,acc=Classifiers.CrossValidate(vec,labels,IDX,cl)
    return acc
...
subTopicsScore(texts,metadata,numtopics=5,name='Brown')
#num_features=300
