...
#fails:
    #subtopic
meta<-read.table('Twitter/Results/TwitterMeta.csv',sep=',',header=T)
#GENERAL ANALYSIS FOR ALL DATA
beh<-read.table('Twitter/rf/Behavior_Preds.csv',sep=',' ,header=T)
raw<-read.table('Twitter/rf/Raw_Preds.csv',sep=',' ,header=T)
rt<-read.table('Twitter/rf/Raw_Topic_Preds.csv',sep=',' ,header=T)
struct<-read.table('Twitter/rf/Structure_Preds.csv',sep=',' ,header=T)
st<-read.table('Twitter/rf/SubTopic_Preds.csv',sep=',' ,header=T)
liwc<-read.table('Twitter/rf/LIWC_Preds.csv',sep=',' ,header=T)
#w2v<-read.table('Twitter/rf/Word2Vec_Preds.csv',sep=',' ,header=T)
#kb<-read.table('Twitter/rf/KBest_Preds.csv',sep=',' ,header=T)
#nw<-read.table('Twitter/rf/Nonwords_Preds.csv',sep=',' ,header=T)
#ind<-read.table('Twitter/rf/Individual_Preds.csv',sep=',' ,header=T)



library(plyr)
#dta<-join_all(list(meta,beh,raw,rt,struct,st,liwc,w2v,kb,nw), by = 'id', type = 'full')
#colnames(dta)<-c('id','sex','party','beh','raw','rawtopic','struct','subtopic','liwc','w2v','kbest','nonwords')

dta<-join_all(list(meta,beh,raw,rt,struct,st,liwc), by = 'id', type = 'full')
colnames(dta)<-c('id','sex','party','beh','raw','rawtopic','struct','subtopic','liwc')
...
accuracies<-function(dta,model){
    ...
    num<-sum((dta$preds>.5 & dta$sex=='m') | (dta$preds<.5& dta$sex=='f'),na.rm=T)
...
saveImportanceChart<-function(fit,file){
    ...
    vars<-rownames(mat)
    jpeg(file)
    barplot(vals, main="Twitter Variable Importance",xlab='Model',ylab='Change in Accuracy',names.arg=vars)
    dev.off()
}

names<-c('raw','rawtopic','w2v','kbest','liwc','nonword','behav.','struct.','subtop.','indiv.')
counts<-c(.69,.58,.57,.64,.52,0,.67,.58,.57,0)
jpeg('Twitter/Results/Twitter_Accuracy_Bar.jpg')
barplot(counts, main="Twitter Classifier Accuracy",ylim=c(0,1),xlab='Model',ylab='Accuracy', cex.names=.6,
  names.arg=names)
dev.off()
#
