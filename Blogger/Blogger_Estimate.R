...
#fails:
    #subtopic
meta<-read.table('Blogger/bloggerMeta.csv',sep=',',header=T)
#GENERAL ANALYSIS FOR ALL DATA
beh<-read.table('Blogger/rf/Behavior_Preds.csv',sep=',' ,header=T)
raw<-read.table('Blogger/rf/Raw_Preds.csv',sep=',' ,header=T)
rt<-read.table('Blogger/rf/Raw_Topic_Preds.csv',sep=',' ,header=T)
struct<-read.table('Blogger/rf/Structure_Preds.csv',sep=',' ,header=T)
st<-read.table('Blogger/rf/SubTopic_Preds.csv',sep=',' ,header=T)
liwc<-read.table('Blogger/rf/LIWC_Preds.csv',sep=',' ,header=T)
w2v<-read.table('Blogger/rf/Word2Vec_Preds.csv',sep=',' ,header=T)
kb<-read.table('Blogger/rf/KBest_Preds.csv',sep=',' ,header=T)
nw<-read.table('Blogger/rf/Nonwords_Preds.csv',sep=',' ,header=T)
ind<-read.table('Blogger/rf/Individual_Preds.csv',sep=',' ,header=T)


library(plyr)
dta<-join_all(list(meta,beh,raw,rt,struct,st,liwc,w2v,kb,nw,ind), by = 'id', type = 'full')
colnames(dta)<-c('id','sex','age','category','sign','beh','raw','rawtopic','struct','subtopic','liwc','w2v','kbest','nonwords','ind')
...
saveImportanceChart<-function(fit,file){
    ...
    vars<-rownames(mat)
    jpeg(file)
    barplot(vals, main="Blogger Variable Importance",xlab='Model',ylab='Change in Accuracy',names.arg=vars)
    dev.off()
}

names<-c('raw','rawtopic','w2v','kbest','liwc','nonword','behav.','struct.','subtop.','indiv.')
counts<-c(.75,.75,.73,.68,.52,.67,.72,.66,.64,.85)
jpeg('Blogger/Results/Blogger_Accuracy_Bar.jpg')
barplot(counts, main="Blogger Classifier Accuracy",ylim=c(0,1),xlab='Model',ylab='Accuracy', cex.names=.6,
  names.arg=names)
dev.off()
#
